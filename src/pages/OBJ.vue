...
const shirtParts = ["Sleeves", "Cuffs", "Buttons", "Front", "Back"];
    ...
    async init() {
      ...
      const loadShirt = async () => {
        ...
        const getPath = name =>
          ...
          }/assets/shirt/${name.toLowerCase()}/${name.toLowerCase()}.drc`;

        return new Promise(resolve => {
          for (const i in shirtParts) {
            dracoLoader.load(getPath(shirtParts[i]), geometry => {
              geometry.computeVertexNormals();
              let obj = new THREE.Mesh(geometry);

              obj.name = shirtParts[i];
              group.add(obj);

              if (group.children.length === shirtParts.length) {
                resolve(group);
              }
            });
          }
